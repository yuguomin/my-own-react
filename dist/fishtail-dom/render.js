...
exports.render = function (element, parentDom) {
    ...
    var isTextELement = type === SpecialChars_1.TEXT_ELEMENT_TYPE;
    // 判断是文本节点还是元素节点，对应生成
    var dom = isTextELement ? document.createTextNode('') : document.createElement(type);
    // 判断是否是一个监听事件
    var isListener = function (name) { return name.startsWith(SpecialChars_1.EVENT_LISTENER_START); };
    Object.keys(props).forEach(function (propName) {
        if (isListener(propName)) {
            // 处理监听器绑定
            var listenerName = propName.toLowerCase().slice(2);
    ...
    childrenELements.forEach(function (child) { return exports.render(child, dom); });
    // 添加生成的 Dom tree 到根元素
